...
from garage.torch.policies import SkillPolicy
# yapf: enable


class SkillSAC(RLAlgorithm):
    ...
    def __init__(
            ...
            *,  # Everything after this is numbers.
            ...
            reward_scale=1.0,
            num_skills=10,
            encoder_hidden_sizes=[400, 400, 400],
            encoder_class=MLPEncoder,
            policy_class=SkillPolicy,
        ...
        skill_in_dim = obs_dim
        skill_encoder = encoder_class(input_dim=skill_in_dim,
                                      output_dim=num_skills,
                                      hidden_sizes=encoder_hidden_sizes)
        
        self.policy = policy_class(skill_encoder=skill_encoder,
                                    policy=inner_policy,
                                    num_skills=num_skills)

        self._policy_optimizer = self._optimizer(self.policy.networks[0].parameters(),
                                                 lr=self._policy_lr)

        discriminator_in_dim = obs_dim + action_dim
        self._discriminator = encoder_class(input_dim=discriminator_in_dim,
                                            output_dim=num_skills,
                                            hidden_sizes=encoder_hidden_sizes)

        self._skill_optimizer = self._optimizer(self.policy.networks[1].parameters(),
                                                lr=encoder_lr)
        ...
        for _ in trainer.step_epochs():
            for _ in range(self._steps_per_epoch):
                ...
                trainer.step_episode = trainer.obtain_samples(
                    trainer.step_itr, batch_size, self.policy)
                path_returns = []
                ...
                for _ in range(self._gradient_steps):
                    policy_loss, qf1_loss, qf2_loss, discriminator_loss = self.train_once()
    ...
    def train_once(self, itr=None, paths=None):
        ...
        if self.replay_buffer.n_transitions_stored >= self._min_buffer_size:
            ...
            policy_loss, qf1_loss, qf2_loss, discriminator_loss = self.optimize_policy(samples)
    ...
    def _actor_objective(self, samples_data, new_actions, skill_embed, log_pi_new_actions):
        ...
        min_q_new_actions = torch.min(self._qf1(torch.cat([obs, skill_embed.detach()], dim=1), new_actions),
                                      self._qf2(torch.cat([obs, skill_embed.detach()], dim=1), new_actions))
        policy_objective = ((alpha * log_pi_new_actions) -
                            min_q_new_actions.flatten()).mean()
        return policy_objective

    def _critic_objective(self, samples_data, skill_embed, log_q_z, log_p_z):
        ...
        rewards += log_q_z.detach().flatten() - log_p_z.detach().flatten()
        with torch.no_grad():
            alpha = self._get_log_alpha(samples_data).exp()

        q1_pred = self._qf1(torch.cat([obs, skill_embed.detach()], dim=1), actions)
        q2_pred = self._qf2(torch.cat([obs, skill_embed.detach()], dim=1), actions)

        policy_out, new_skill_dist, new_skill_embed = self.policy(next_obs)
        new_next_actions_dist = policy_out[0]
        ...
        target_q_values = torch.min(
            self._target_qf1(torch.cat([next_obs, skill_embed.detach()], dim=1), new_next_actions),
            self._target_qf2(
                torch.cat([next_obs, skill_embed.detach()], dim=1), new_next_actions)).flatten() - (alpha * new_log_pi)
        with torch.no_grad():
    ...
    def _discriminator_log_likelihood(self, samples_data, new_actions, skill_embed):
        obs = samples_data['observation']
        discriminator_out = self._discriminator(torch.cat([obs, new_actions.detach()], dim=1))
        q_z = F.softmax(discriminator_out, dim=1)
        log_q_z = torch.log((q_z * skill_embed.detach()).sum(dim=-1, keepdim=True) + 1e-8)
        return log_q_z, discriminator_out
    ...
    def optimize_policy(self, samples_data):
        ...
        log_q_z, discriminator_out = self._discriminator_log_likelihood(samples_data, new_actions, skill_embed)
        qf1_loss, qf2_loss = self._critic_objective(samples_data, skill_embed, log_q_z, log_p_z)
        ...
        policy_loss = self._actor_objective(samples_data, new_actions,
                                            skill_embed, log_pi_new_actions)
        zero_optim_grads(self._policy_optimizer)
        zero_optim_grads(self._skill_optimizer)
        policy_loss.backward()

        self._policy_optimizer.step()
        self._skill_optimizer.step()
        ...
        zero_optim_grads(self._discriminator_optimizer)
    ...
    def _log_statistics(self, policy_loss, qf1_loss, qf2_loss, discriminator_loss):
        ...
        tabular.record('Discriminator/Loss', discriminator_loss.item())
    ...
    def networks(self):
        ...
        return self.policy.networks + [self.policy] + [
            self._qf1, self._qf2, self._target_qf1,
